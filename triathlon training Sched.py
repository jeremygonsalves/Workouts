...
for i, day in enumerate(days):
    ...
    for activity in activities[i]:
        time_label = tk.Label(window, text=activity)
        time_label.pack()
        var = tk.StringVar(window)
        var.set(times[0])
        dropdown = ttk.Combobox(window, textvariable=var, values=times)
        dropdown.pack()
        dropdowns.append(dropdown)
    entries.append(dropdowns)

# Create the generate button
generate_button = tk.Button(window, text="Generate Schedule", command=generate_calendar)
